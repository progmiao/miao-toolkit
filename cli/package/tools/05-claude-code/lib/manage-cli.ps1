    ...
    try {
        ...
        if (-not $installed) {
            ...
            Set-ToolkitDepOperationInFlightStatus -Ui $ui -MainText (Get-ClaudeCodeI18n -ToolRoot $ToolRoot `
                -Key 'claude-code.cli.detectUpdatePreflight') -AdvanceSpinner
            & $RedrawView
            & $pump

            & $fnWriteLog -Log $log -Text (Get-ClaudeCodeI18n -ToolRoot $ToolRoot `
                -Key 'claude-code.cli.detectUpdatePreflight') -Kind 'text' -WithTimestamp
            ...
            $updateCheck = Resolve-WingetPackageUpdateCheck -PackageId (Get-ClaudeCodeWingetPackageId) `
                -InstalledVersion $versionLabel -OnPulse $pump
            & $pump

            if ($updateCheck.Skipped) {
                $skipKey = if ([string]$updateCheck.SkipReason -eq 'timeout') {
                    'claude-code.cli.detectUpdateSkippedTimeout'
                }
                else {
                    'claude-code.cli.detectUpdateSkippedNetwork'
                }
                & $fnApplyProgress -Ui $ui -WingetOutputState $wingetOutputState `
                    -Decision @{ ProgressPhase = 'wingetSuccess'; RedrawOnly = $true }
                & $fnWriteLog -Log $log -Text (Get-ClaudeCodeI18n -ToolRoot $ToolRoot -Key $skipKey `
