...
| **首页工具列表** | 不展示依赖状态（装/卸在工具内或 `miao install`） |
...
**第三方依赖：** 装 Miao 时不一并安装；用户通过 **依赖管理专页**（`miao install`）或 **工具内菜单** 主动装/升。首页不展示依赖状态。详见 §四。
