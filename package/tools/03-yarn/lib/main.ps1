...
function Get-YarnToolSectionTitle {
    return (Get-ToolSectionTitle -Tool $tool)
}

function Resolve-YarnActionScript {
    param([string]$RelativePath)
    Join-Path $ToolRoot ($RelativePath -replace '/', [IO.Path]::DirectorySeparatorChar)
}

function Invoke-YarnAction {
    param($Action)

    $scriptPath = Resolve-YarnActionScript $Action.script
...
$sectionTitle = Get-YarnToolSectionTitle
...
while ($true) {
    ...
    if ($probeChanged -or -not $ToolkitShell.HeaderLocale -or $ToolkitShell.HeaderLocale -ne $currentLocale) {
        Clear-ShellSingleSelectListCache -Shell $ToolkitShell -CacheKey 'Yarn'
        $sectionTitle = Get-YarnToolSectionTitle
    }
