...
function Get-PnpmToolSectionTitle {
    return (Get-ToolSectionTitle -Tool $tool)
}

function Resolve-PnpmActionScript {
    param([string]$RelativePath)
    Join-Path $ToolRoot ($RelativePath -replace '/', [IO.Path]::DirectorySeparatorChar)
}

function Invoke-PnpmAction {
    param($Action)

    $scriptPath = Resolve-PnpmActionScript $Action.script
...
$sectionTitle = Get-PnpmToolSectionTitle
...
while ($true) {
    ...
    if ($probeChanged -or -not $ToolkitShell.HeaderLocale -or $ToolkitShell.HeaderLocale -ne $currentLocale) {
        Clear-ShellSingleSelectListCache -Shell $ToolkitShell -CacheKey 'Pnpm'
        $sectionTitle = Get-PnpmToolSectionTitle
    }
