...
public sealed class InstallerLaunchHandler : IToolActionHandler
{
    ...
    public async Task<JobResult> ExecuteAsync(ToolActionContext context, CancellationToken cancellationToken = default)
    {
        ...
        {
            return new JobResult(context.JobId, false, 1, "plugin.json 缺少 install.downloadUrl");
        }
...
public sealed class WingetInstallHandler : IToolActionHandler
{
    ...
    public async Task<JobResult> ExecuteAsync(ToolActionContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (install is null || string.IsNullOrWhiteSpace(install.PackageId))
            return new JobResult(context.JobId, false, 1, "plugin.json 缺少 install.packageId");
        ...
        sb.AppendLine($"Write-Host 'winget install --id {Escape(install.PackageId!)} -e --accept-package-agreements --accept-source-agreements{source}'");
        sb.AppendLine($"winget install --id {Escape(install.PackageId!)} -e --accept-package-agreements --accept-source-agreements{source}");
        sb.AppendLine("if ($LASTEXITCODE -ne 0) { throw \"winget 失败 exit=$LASTEXITCODE\" }");
        sb.AppendLine("Write-Host '##progress 100'");
...
public sealed class NpmGlobalInstallHandler : IToolActionHandler
{
    ...
    public async Task<JobResult> ExecuteAsync(ToolActionContext context, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(pkg))
            return new JobResult(context.JobId, false, 1, "plugin.json 缺少 install.npmPackage");
