...
启动期做**开库 / 种子 / 服务构造 / 安装态校准**；若 `package_versions` 为空则**首次**拉远端版本目录。  
**更新探测**与**已有缓存后的版本目录增量**进主壳后由静默任务补跑。
...
| `boot.taskTimeoutSeconds` | 单任务超时（秒）；首次版本目录同步单独放宽至 3 分钟 | 30 | 3–600 |

- **核心任务**（开库、初始化服务）：达上限 → 关窗退出  
- **数据任务**（种子、安装态、首次版本目录）：达上限 → 跳过该项，仍进主壳（降级）  
...
两者互不影响；开发每次都能从启动页看到全流程（灌库、进度日志等）。开发库每次重建会导致版本目录始终走「首次同步」（在初始化内拉取）。
...
| id | 含义 |
|----|------|
| `detect.update` | 检查已装工具是否有更新（后台） |
| `cache.versions.node` / `pnpm` / `yarn` | 版本目录增量同步（仅启动时库中**已有**缓存的包入队；本次启动刚首次灌入的不入队） |

安装态校准在启动初始化完成；`package_versions` 为空时的首次远端同步也在初始化完成。
